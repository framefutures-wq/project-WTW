...
- 사용자가 새 채팅에서 `계속`이라고만 말해도 갈틈 작업 재개 신호로 취급한다.
- 먼저 최신 프로젝트 상태를 복구·확인한 뒤, 사용자에게 짧게 다음 4가지를 보고한다.
  1. **완료된 작업** — 어디까지 끝났는지
  2. **현재 작업** — 지금 이어서 처리 중인 bounded task가 무엇인지
  3. **남은 작업** — 큰 덩어리 기준으로 몇 개/무엇이 남았는지
  4. **사용자 필요 여부** — 지금 당장 사용자 조작이 필요한지
- 상태 보고 직후 별도 승인 없이 다음 안전한 bounded task를 바로 이어서 진행한다.
- 완료/현재/남은 수치는 문서 기억이 아니라 latest main/실제 production 상태를 우선해 계산한다.
