...
export const SELECTED_EVENTS = [
  ...
  },
  {
    ...
    programs: [{ id: "royalguard-2026-change", name: "교대의식", description: "수문장과 수문군의 근무 교대를 재현", featured: 1, occurrences: [["2026-09-20", "11:00", "11:30", "덕수궁 대한문", "월요일 제외 매일 1회"], ["2026-09-20", "14:00", "14:30", "덕수궁 대한문", "월요일 제외 매일 2회"]], tags: ["performance"] }, { id: "royalguard-2026-guard", name: "수위의식", description: "교대의식 전후 대한문 수위의식", featured: 0, occurrences: [["2026-09-20", "10:00", "15:30", "덕수궁 대한문", "월요일 제외" ]], tags: ["performance"] }, { id: "royalguard-2026-patrol", name: "순라의식", description: "교대 뒤 궁궐과 도성 내외를 순찰", featured: 0, occurrences: [["2026-09-20", "15:00", "16:00", null, "화·목 남대문시장 / 수·금·토·일 의정부 터"]], tags: ["parade"] }],
  ...
  },
  {
    ...
    programs: [{ id: "sungnyemun-2026-opening", name: "숭례문 개문의식", description: "숭례문을 여는 의식", featured: 1, occurrences: [["2026-09-20", "10:00", "10:10", "숭례문 광장", "월요일 제외 매일"]], tags: ["performance"] }, { id: "sungnyemun-2026-guard", name: "파수의식", description: "숭례문을 수위하는 파수의식", featured: 0, occurrences: [["2026-09-20", "10:10", "12:00", "숭례문 광장", "12:00~13:00 휴식"], ["2026-09-20", "13:00", "15:30", "숭례문 광장", "월요일 제외 매일"]], tags: ["performance"] }, { id: "sungnyemun-2026-closing", name: "숭례문 폐문의식", description: "숭례문을 닫는 의식", featured: 0, occurrences: [["2026-09-20", "15:30", "15:40", "숭례문 광장", "월요일 제외 매일"]], tags: ["performance"] }],
...
export function buildSql() {
  ...
  for (const event of SELECTED_EVENTS) {
    ...
    event.programs.forEach((program, index) => {
      statements.push(`INSERT INTO event_programs(id,event_id,program_name,program_date,start_time,end_time,schedule_text,venue_name,description,featured,sort_order,source_id,evidence_excerpt,updated_at) VALUES(${quote(program.id)},${quote(event.id)},${quote(program.name)},NULL,NULL,NULL,NULL,NULL,${quote(program.description)},${program.featured ? 1 : 0},${index + 1},${quote(event.source.id)},${quote(event.evidence)},${quote(NOW)}) ON CONFLICT(id) DO UPDATE SET program_name=excluded.program_name,description=excluded.description,featured=excluded.featured,sort_order=excluded.sort_order,source_id=excluded.source_id,evidence_excerpt=excluded.evidence_excerpt,updated_at=excluded.updated_at;`);
