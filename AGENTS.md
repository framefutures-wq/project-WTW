...
작업을 시작할 때 루트의 `PROJECT_CONTEXT.md`를 먼저 읽고 현재 브랜드, 공식 도메인, 완료된 단계, 운영 원칙, 현재 우선순위를 확인한다. 또한 `docs/WORKING_RULES.md`를 읽고 프롬프트 작성 방식, 모델 선택, bounded task, 테스트·CI·배포·최종보고 규칙을 적용한다. UI/상세 작업이면 `docs/UI_V2_DIRECTION.md`도 반드시 읽는다. 이 파일은 대화 전체 로그가 아니라 새 세션에서 바로 이어가기 위한 현재 상태 요약이다.

중요한 제품 결정, 공식 도메인, 운영 상태, 완료 Phase, 다음 우선순위가 바뀌는 작업을 정상 완료하면 `PROJECT_CONTEXT.md`도 함께 갱신한다. 단순 문구 수정이나 사소한 구현 변경마다 불필요하게 장황하게 늘리지 않는다. GitHub의 최신 `main`과 실제 production 상태가 문서보다 우선하며, 값이 변할 수 있는 commit hash·row count 등은 작업 시작 시 다시 검증한다.
