...
function loadCandidates() {
  ...
  if (unknown.length || duplicate.length)
    throw new Error(
      `invariant failed: unknown=${unknown.length}, duplicate=${duplicate.length}`,
    );
  if (
    candidates.some((c) => c.tag === "parking" || c.tag === "pet_not_allowed")
  )
    throw new Error("excluded tag generated");
...
if (mode === "verify")
  requireRemoteReadApproval(
    [...process.argv.slice(3), "--remote"],
    "fact-tag-operations verify",
  );
...
const tagCounts = Object.fromEntries(
  FACT_TAGS.map((tag) => [
    tag,
    new Set(candidates.filter((c) => c.tag === tag).map((c) => c.event_id))
      .size,
  ]),
);
const summary = {
  event_count: report.snapshot.event_count,
  tagged_events: report.tagged_events,
  untagged_events: report.untagged_events,
  candidate_rows: candidates.length,
  event_ids_with_tags: eventIds.size,
  average_tags: report.snapshot.event_count
    ? candidates.length / report.snapshot.event_count
    : 0,
  max_tags: Math.max(
    0,
    ...[...eventIds].map(
      (id) => candidates.filter((c) => c.event_id === id).length,
    ),
  ),
  ...
  unknown_tags: 0,
  excluded_tags: 0,
  classifier_type: FACT_CLASSIFIER,
  rule_version: FACT_RULE_VERSION,
};
...
if (mode === "apply") {
  ...
  for (let offset = 0; offset < ids.length; offset += 25) {
    ...
    const chunk = candidates.filter((candidate) =>
      selected.has(candidate.event_id),
    );
...
if (mode === "verify") {
  const rows = query(
    `SELECT event_id,tag,classifier_type,rule_version,rule_id,evidence_source_ref,evidence_field,evidence_excerpt FROM event_tags WHERE classifier_type=${quote(FACT_CLASSIFIER)} AND rule_version=${quote(FACT_RULE_VERSION)} ORDER BY event_id,tag`,
  );
  const actual = new Set(rows.map((r) => `${r.event_id}|${r.tag}`));
  const expected = new Set(candidates.map((c) => `${c.event_id}|${c.tag}`));
  const orphan = query(
    "SELECT t.event_id FROM event_tags t LEFT JOIN events e ON e.id=t.event_id WHERE e.id IS NULL",
  );
  const invalid = rows.filter((r) => !V1.has(r.tag));
  const missing = [...expected].filter((key) => !actual.has(key));
  const extra = [...actual].filter((key) => !expected.has(key));
  if (missing.length || extra.length || invalid.length || orphan.length)
    throw new Error(
      `verification failed: missing=${missing.length}, extra=${extra.length}, invalid=${invalid.length}, orphan=${orphan.length}`,
    );
  summary.actual_rows = rows.length;
  summary.missing = missing.length;
  summary.extra = extra.length;
  summary.orphan = orphan.length;
