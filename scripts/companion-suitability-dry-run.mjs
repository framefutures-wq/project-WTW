...
if (!snapshotArg || !existsSync(snapshotArg))
  throw new Error(
    "companion dry-run은 remote D1을 읽지 않습니다. `npm run fact-tags:snapshot` 후 local fact-tag report를 사용하거나 --snapshot <file>을 지정하세요.",
  );
const stored = JSON.parse(readFileSync(snapshotArg, "utf8"));
const factReport = JSON.parse(
  readFileSync(".wrangler/fact-tag-dry-run.json", "utf8"),
);
const events = stored.events
  .filter((event) => !event.is_sample)
  .map((event) => ({ id: event.id, title: event.title }));
const tags = factReport.candidates.map((candidate) => ({
  event_id: candidate.event_id,
  tag: candidate.tag,
  rule_id: candidate.rule_id,
  evidence_source_ref: candidate.evidence_source,
  evidence_field: candidate.field,
  evidence_excerpt: candidate.evidence_text,
}));
const snapshotSource = snapshotArg;
const byEvent = new Map(events.map((event) => [event.id, new Map()]));
for (const row of tags)
  if (byEvent.has(row.event_id)) byEvent.get(row.event_id).set(row.tag, row);
...
function has(map, tag) {
  return map.has(tag);
}
function firstCombo(map, combos) {
  return combos.find(([pair]) => pair.every((tag) => has(map, tag)));
}
function sourceFacts(map, selected) {
  return selected.map((tag) => {
    ...
    return {
      ...
      evidence_source: row?.evidence_source_ref ?? null,
      evidence_field: row?.evidence_field ?? null,
...
function result(
  ...
  return {
    ...
    positive_reason_codes: reason ? [reason] : [],
    caution_reason_codes: caution ? [caution] : [],
    source_fact_tags: sourceFacts(map, selected),
    rule_id,
    rule_version: SUITABILITY_VERSION,
  };
}
function classify(map) {
  ...
  if (childStrong)
    ...
    childFamily &&
    ["experience", "education", "sports", "exhibition", "nature_scenery"].some(
      (tag) => has(map, tag),
    )
  )
    child = result(
      ...
      [
        "family_program",
        ...["experience", "education", "sports", "exhibition", "nature_scenery"]
          .filter((tag) => has(map, tag))
          .slice(0, 1),
      ],
  ...
  const couple =
    couplePair && couplePair[1] === "couple.food_local"
      ? result(
          "couple",
          "unknown",
          null,
...
for (const event of events)
  for (const row of classify(byEvent.get(event.id)))
    rows.push({ event_id: event.id, title: event.title, ...row });
const companions = ["child", "couple", "parents", "pet"];
const states = {
  child: ["fit", "conditional", "unknown"],
  couple: ["fit", "conditional", "unknown"],
  parents: ["fit", "conditional", "unknown"],
  pet: ["allowed", "unknown"],
};
const distribution = Object.fromEntries(
  companions.map((companion) => [
    companion,
    Object.fromEntries(
      states[companion].map((state) => [
        state,
        rows.filter(
          (row) =>
            row.companion_type === companion && row.suitability_state === state,
        ).length,
      ]),
    ),
  ]),
...
for (const row of rows)
  if (
    (row.companion_type !== "pet" && row.suitability_state === "fit") ||
    (row.companion_type === "pet" && row.suitability_state === "allowed")
  )
    fitByEvent.set(row.event_id, fitByEvent.get(row.event_id) + 1);
const multiCompanion = Object.fromEntries(
  [0, 1, 2, 3, 4].map((n) => [
    String(n),
    [...fitByEvent.values()].filter((value) => value === n).length,
  ]),
);
const rulePathCounts = Object.fromEntries(
  [...new Set(rows.map((row) => `${row.companion_type}|${row.rule_id}`))]
    .sort()
    .map((key) => [
      key,
      rows.filter((row) => `${row.companion_type}|${row.rule_id}` === key)
        .length,
    ]),
);
const qa = {};
for (const companion of companions) {
  ...
  for (const state of states[companion]) {
    const limit =
      companion === "parents" && state === "unknown"
        ? 30
        : companion === "parents" && state === "fit"
          ? 99
          : 12;
    qa[companion][state] = rows
      .filter(
        (row) =>
          row.companion_type === companion && row.suitability_state === state,
      )
      ...
      .map((row) => ({
        ...
        title: row.title,
        source_fact_tags: row.source_fact_tags.map((fact) => fact.tag),
        reason: row.positive_reason_codes,
        caution: row.caution_reason_codes,
        rule_id: row.rule_id,
      }));
  }
}
const qaEventIds = new Set(
  Object.values(qa).flatMap((byState) =>
    Object.values(byState).flatMap((sample) =>
      sample.map((row) => row.event_id),
    ),
  ),
);
const report = {
  ...
  snapshot: {
    ...
    tagged_event_count: byEvent.size
      ? [...byEvent.values()].filter((map) => map.size).length
      : 0,
    event_tag_rows: tags.length,
    classifier_type: CLASSIFIER,
    fact_rule_version: VERSION,
    source: snapshotSource,
  },
  rule_version: SUITABILITY_VERSION,
  rules: {
    child: {
      strong: ["children_program"],
      combinations: childCombo.map(([pair, rule]) => ({ tags: pair, rule })),
    },
    couple: {
      combinations: couplePairs.map(([pair, rule]) => ({ tags: pair, rule })),
    },
    parents: { content: [...parentContent], comfort: [...parentComfort] },
    pet: { allowed_tag: "pet_allowed", no_inference: true },
  },
  distribution,
  rule_path_counts: rulePathCounts,
  multi_companion_fit: multiCompanion,
  qa_event_count: qaEventIds.size,
  rows,
  qa_samples: qa,
  qa_findings: {
    resolved_weak_combination: {
      pattern: "food+local_specialty",
      affected_events: rows.filter(
        (row) =>
          row.companion_type === "couple" &&
          row.caution_reason_codes.includes(
            "couple.couple_specificity_missing",
          ),
      ).length,
      action: "fit/conditional에서 unknown으로 유지",
    },
    remaining_false_positive_candidates: [],
    false_negative_candidates: [],
  },
  unknown_rows: rows.filter((row) => row.suitability_state === "unknown"),
};
mkdirSync("/tmp/wtw-companion", { recursive: true });
writeFileSync(
  "/tmp/wtw-companion/report.json",
  JSON.stringify(report, null, 2),
);
writeFileSync(
  ".wrangler/companion-suitability-dry-run.json",
  JSON.stringify(report, null, 2),
);
console.log(
  JSON.stringify(
