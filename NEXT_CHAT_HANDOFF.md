...
- detail run message에 `retry_attempted`, `retry_recovered`, `retry_exhausted`, `failure_endpoints` sanitized 집계를 남긴다.
- 다음 작업은 production detail collection 1회를 실행해 inline retry 복구율과 잔여 failure 원인을 감사하는 것이다.
