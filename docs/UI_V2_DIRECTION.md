...
## 13. 2026-09-26 홈 컨트롤 밀도 / 중복 필터 정리

- 빠른 카테고리를 홈의 **단일 theme/category 선택 수단**으로 사용한다. 같은 theme 목록을 세부필터 안에서 다시 반복하지 않는다.
- 빠른 카테고리 선택만으로 세부필터 패널을 자동으로 열지 않는다. 선택 상태는 상단 카테고리 자체에서 보여주며, 같은 값을 `현재 조건` 칩으로 다시 반복하지 않는다.
- 세부필터는 동행 대상·위치 관련 상태·알림처럼 상단 quick category와 겹치지 않는 보조 조건만 담당한다.
...
## 15. 2026-09-27 홈 타이포 / 설명문 밀도 기준

- Klook/Fever/GetYourGuide 공통처럼 홈의 카테고리·날짜 컨트롤은 짧은 라벨 중심으로 유지한다.
- `빠르게 둘러보기` 같은 kicker와 `관심 있는 주제를 누르면...` 같은 설명문을 카테고리 바로 위에 중복하지 않는다.
- quick category는 symbol + label만 보여주며 `시장 · 푸드`, `공연 · 무대` 같은 helper line은 홈에서 제거한다.
