...
export function assessCost(
  ...
  const years = [...text.matchAll(/\b(20\d{2})년/g)].map((match) => Number(match[1]));
  if (years.some((year) => year < referenceYear) && !years.includes(referenceYear))
    return { status: "unknown", reason: "stale_price_text" };
