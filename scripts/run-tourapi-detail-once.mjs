...
try {
  ...
  const alias = `detail-once-${randomUUID().replace(/-/g, "").slice(0, 10)}`;
  const uploaded = run(["versions", "upload", "--config", tempConfig, "--keep-vars", "--preview-alias", alias, "--message", "detail-only one-shot"]);
  ...
  console.log(JSON.stringify({ manualRunId, alias, url, result: await response.json() }));
