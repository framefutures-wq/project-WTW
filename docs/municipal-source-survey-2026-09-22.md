...
| Ulsan        | `https://ulsan.go.kr/y/yes/main.do`                                      | Deferred after featured-list audit  | Featured cards use two-digit years only, detail links point to `ulsanculture.kr`, sessionized links are mixed into the document, and the block is a partial swiper rather than a durable full event list. Current generic extraction therefore remains fail-closed. One final read-only search for a separate full-year canonical Ulsan list is allowed before excluding the region candidate. |
